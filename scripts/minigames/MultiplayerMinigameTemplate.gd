...
#############################
# This script is a template for multiplayer minigames and should be 
# attached to the corresponding scene template MultiplayerMiniGameTemplate.
#
# To use this template, duplicate the scene template and this script template
# and attach the duplicated script to the duplicated scene's root node.
#
# Only things to work on for the new minigame is the GameWindow node and
# any game-specific logic in the script.
# 
# Without changing anything from the templates, player can enter collision
# shape, display game prompt and game window (black rectangle) and leave
# collision zone to hide everything again. 
# Game result will only display once server_submit_move is implemented and the
# game's win condition is checked.
# Also note that _on_body_entered and _on_body_exited have a check if the player
# is in multiplayer mode (currently a host or joiner to a server). If using
# this template script for a single player minigame, remove that check or else
# the collision will not register.
