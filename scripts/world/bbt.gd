...
@export var bbt_exit_polygon: Polygon2D
...
func _ready() -> void:
	...
	exit_button.pressed.connect(_on_bbt_exit_pressed)

func _on_bbt_exit_pressed() -> void:
...
func highlight_bbt_exit():
	var flash_tween = create_tween()
	bbt_exit_polygon.modulate.a = 0.5
	flash_tween.tween_property(bbt_exit_polygon, "modulate:a", 0.0, 0.25).set_trans(Tween.TRANS_QUAD).set_ease(Tween.EASE_OUT)
